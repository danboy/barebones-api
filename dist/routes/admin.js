...
function defineEndpoints({ mw, ctrlrs }) {
    ...
    router
        .route("/roles/:slug")
        .get(mw.localRoleBySlug({ includeActor: true, loadUsers: true }), ctrlrs.Roles.show())
    ...
    router
        ...
        .put(mw.localRoleBySlug({ attrName: "roleSlug" }), mw.localUserBySlug({ includeActor: true }), ctrlrs.RoleAssignments.create())
        .delete(mw.localRoleBySlug({ attrName: "roleSlug" }), mw.localUserBySlug({ includeRoles: true }), ctrlrs.RoleAssignments.delete());
    return router;
}
